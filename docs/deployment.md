...
5. Note the **IP address** (e.g. `178.62.100.50`) — needed in the next steps
...
```bash
ssh-copy-id root@178.62.100.50

# Verify it works (should not ask for a password)
ssh root@178.62.100.50 "echo connected"
```
...
| Type | Name | Value | TTL |
| :--- | :--- | :--- | :--- |
| `A` | `@` | `178.62.100.50` | Auto |
| `A` | `www` | `178.62.100.50` | Auto |
...
Edit `config/deploy.yml` and replace the placeholders with your Droplet IP:
...
servers:
  web:
    hosts:
      - 178.62.100.50    # ← your actual Droplet IP
...
accessories:
  postgres:
    host: 178.62.100.50  # ← same IP again
```

Everything else in `config/deploy.yml` is already configured:
...
| `DATABASE_URL` | `postgresql://visasponsoruk:${DB_PASSWORD}@visasponsoruk-postgres:5432/visasponsoruk_production` |
...
Expected output:
```
INFO [kamal] Running kamal setup on 178.62.100.50
INFO [kamal] Starting accessory postgres on 178.62.100.50
INFO [kamal] Deploying app containers to 178.62.100.50
...
scp root@178.62.100.50:/root/backup_*.sql ~/Desktop/
```
...
| File | Purpose |
| :--- | :--- |
| `Dockerfile` | Multi-stage production image — thruster, jemalloc, asset precompile |
| `config/deploy.yml` | Kamal 2 config — servers, proxy, PostgreSQL accessory |
| `.kamal/secrets` | Secret variable names (safe to commit — no raw values) |
| `bin/docker-entrypoint` | Runs `db:prepare` before server starts on each boot |
...
| `DATABASE_URL` | Built from `DB_PASSWORD` in deploy.yml | `config/deploy.yml` |
